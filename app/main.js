...
function createUserLocation(){
 userLocationLayer=new THREE.Group();userLocationLayer.name='我的实时位置';userLocationLayer.visible=false;scene.add(userLocationLayer);
 const accuracyMaterial=new THREE.MeshBasicMaterial({color:0x2689c7,transparent:true,opacity:.13,depthWrite:false,side:THREE.DoubleSide});
 userLocationAccuracy=new THREE.Mesh(new THREE.CircleGeometry(1,48),accuracyMaterial);userLocationAccuracy.rotation.x=-Math.PI/2;userLocationAccuracy.renderOrder=8;userLocationLayer.add(userLocationAccuracy);
 const ring=new THREE.Mesh(new THREE.RingGeometry(.72,1,32),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:.95,depthWrite:false,side:THREE.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=.12;ring.renderOrder=9;userLocationLayer.add(ring);
 const dot=new THREE.Mesh(new THREE.SphereGeometry(.56,20,12),new THREE.MeshBasicMaterial({color:0x2187c7,depthWrite:false}));dot.position.y=.4;dot.renderOrder=10;userLocationLayer.add(dot);
...
function pauseLocationFollow(){if(locationWatchId!==null&&locationFollowing){locationFollowing=false;locationHasCentered=false;updateLocationButton('paused');}}
function focusUserLocation(){
 if(!locationInside||!userLocationTarget)return;
 selected=null;selectedStation=null;transportLayer?.select(null);$('#detail').hidden=true;updateFraming();overview=false;topView=false;$('#topview').setAttribute('aria-pressed','false');
 ...
 const distance=Math.max(42,Math.min(90,Math.max(terrain.width,terrain.depth)*.14));animateTo(userLocationTarget,new THREE.Vector3(distance*.46,distance*.64,distance),850);locationHasCentered=true;
}
function updateUserLocation(position){
 const {longitude,latitude,accuracy}=position.coords,p=pos(longitude,latitude),inside=Math.abs(p.x)<=halfX&&Math.abs(p.z)<=halfZ;
 locationAccuracy=Number.isFinite(accuracy)?accuracy:null;locationInside=inside;
 if(!inside){if(userLocationLayer)userLocationLayer.visible=false;if(userLocationMarker)userLocationMarker.el.style.visibility='hidden';$('#status').textContent='当前位置在当前沙盘外';$('#live').textContent='当前位置不在当前景区沙盘范围内';return;}
...
function stopLocationTracking(announce=true){
 if(locationWatchId!==null)navigator.geolocation.clearWatch(locationWatchId);locationWatchId=null;locationFollowing=false;locationInside=false;locationHasCentered=false;locationAccuracy=null;userLocationPoint=null;userLocationTarget=null;
 if(userLocationLayer)userLocationLayer.visible=false;if(userLocationMarker)userLocationMarker.el.style.visibility='hidden';updateLocationButton('inactive');
 if(announce){$('#status').textContent='实时定位已关闭';$('#live').textContent='实时定位已关闭';}
}
function handleLocationError(error){
 const message=error.code===1?'未获得定位权限，请在浏览器设置中允许访问位置':error.code===2?'暂时无法获取位置，请到开阔处重试':'定位超时，请稍后重试';
 if(error.code===1)stopLocationTracking(false);$('#status').textContent=message;$('#live').textContent=message;
}
function startLocationTracking(){
 if(!navigator.geolocation){const message='当前浏览器不支持定位';$('#status').textContent=message;$('#live').textContent=message;return;}
 locationFollowing=true;locationHasCentered=false;updateLocationButton('following');$('#status').textContent='正在获取位置…';$('#live').textContent='正在请求设备定位';
 try{locationWatchId=navigator.geolocation.watchPosition(updateUserLocation,handleLocationError,{enableHighAccuracy:true,maximumAge:3000,timeout:15000});}
 catch(error){console.error(error);stopLocationTracking(false);$('#status').textContent='无法启动定位，请检查浏览器权限';}
}
...
function init(){try{
 ...
 controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.dampingFactor=.065;controls.minDistance=4;controls.maxDistance=1600;controls.minPolarAngle=.03;controls.maxPolarAngle=Math.PI*.455;controls.target.copy(homeTarget);controls.autoRotateSpeed=.45;controls.panSpeed=.8;controls.screenSpacePanning=false;controls.zoomSpeed=.8;controls.addEventListener('start',()=>{overview=false;pauseLocationFollow();cancelFlight();});controls.addEventListener('end',()=>{$('#status').textContent=locationStatusText()||destinationName()||'自由探索';});
...
function render(now){requestAnimationFrame(render);if(document.hidden)return;
 if(flight){const t=Math.min(1,(now-flight.start)/flight.duration),e=smooth(t);camera.position.lerpVectors(flight.from,flight.to,e);camera.position.y+=Math.sin(Math.PI*t)*Math.min(65,flight.from.distanceTo(flight.to)*.12);controls.target.lerpVectors(flight.fromTarget,flight.target,e);if(t>=1){flight=null;$('#status').textContent=locationStatusText()||(destinationName()?`已抵达 · ${destinationName()}`:CONFIG.name+'全景');$('#live').textContent=$('#status').textContent;}}
...
$('#locate-me').onclick=()=>{if(!ready)return;if(locationWatchId===null)startLocationTracking();else if(!locationFollowing){locationFollowing=true;updateLocationButton('following');focusUserLocation();}else stopLocationTracking();};
